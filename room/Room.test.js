...
describe("Room", () => {
  describe("constructor", () => {
    ...
    test("should throw an error if the id is undefined or less than 0", () => {
      expect(() => new Room(undefined, "room1", 2)).toThrow(
        new Error(INVALID_ID)
      );
      expect(() => new Room(-1, "room1", 2)).toThrow(new Error(INVALID_ID));
    });
    ...
    test("should throw an error if the room_size is undefined or less than 2", () => {
      expect(() => new Room(1, "room1", 1)).toThrow(
        new Error(INVALID_ROOM_SIZE)
      );
      expect(() => new Room(1, "room1", -1)).toThrow(
        new Error(INVALID_ROOM_SIZE)
      );
