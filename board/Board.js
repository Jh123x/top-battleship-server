...
export class Board {
  ...
  placeShip(ship, x, y, isVertical) {
    ...
    if (
      ...
      (isVertical && this.isOutOfBounds(x + ship.getSize(), y)) ||
      (!isVertical && this.isOutOfBounds(x, y + ship.getSize()))
    )
      throw new Error(BOARD_OUT_OF_BOUNDS);

    // Check if the ship overlaps with another ship
    if (!this.isValidPosition(x, y, ship.getSize(), isVertical))
      throw new Error(BOARD_INVALID_POSITION);
   ...
   * @returns {boolean}
   */
  isValidPosition(x, y, size, isVertical) {
    if (this.isOutOfBounds(x, y)) return false;
    return isVertical
      ? this.isValidVerticalPosition(x, y, size)
      : this.isValidHorizontalPosition(x, y, size);
  }
  ...
  isValidHorizontalPosition(x, y, size) {
    if (this.isOutOfBounds(x, y) || this.isOutOfBounds(x + size, y))
      return false;
    for (let i = 0; i < size; ++i) {
  ...
  isValidVerticalPosition(x, y, size) {
    if (this.isOutOfBounds(x, y) || this.isOutOfBounds(x, y + size))
      return false;
    for (let i = 0; i < size; ++i) {
